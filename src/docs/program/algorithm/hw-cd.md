...
let res = ref('')
let inputSrt = ref('1,1,0,0,1,1,1,0,1')
function fn(inputSrt) {
if (inputSrt=='') return alert('请输入！')
  // 将所有，去掉
  let cars = inputSrt.replaceAll(',','')
...
<template>
  <p>多次输入用空格隔开</p>
  <input v-model='inputSrt' />
  <button @click='fn(inputSrt)' >点击运行</button>
  <h1>代码运行结果：{{ res }}</h1>
</template>
...
let inputSrt = ref('')
function fn(text, inputSrt) {
  if (inputSrt=='') return alert('请输入！')
  // 分割所有的非字母，去重
  let arr = [...new Set(text.split(/[^a-zA-Z]/))]
  // startsWith：是否以inputSrt开头的单词
  // 过滤出目标单词，排序
  res.value = arr.filter((i)=>{
    return i.startsWith(inputSrt) 
  }).sort().join(' ')
}
</script>
...
<template>
  ...
  <input v-model='inputSrt' />
  <button @click='fn(text,inputSrt)' >点击运行</button>
  <h1>代码运行结果：{{ res }}</h1>
</template>
...
let res = ref('')
let num = ref('5')
let inputSrt = ref('2 computing')
let apis = ref('/huawei/computing/no/one /huawei/computing /huawei /huawei/cloud/no/one /huawei/wireless/no/one')
function fn(num,inputSrt,apis) {
if (inputSrt=='') return alert('请输入！')
  let level = inputSrt.split(' ')[0]
  let match = inputSrt.split(' ')[1]
...
<template>
  ...
  <input v-model='inputSrt' /><br>
  <button @click='fn(num,inputSrt,apis)' >点击运行</button>
  <h1>代码运行结果：{{ res }}</h1>
</template>
...
if (inputSrt=='') return alert('请输入！')
  const reg = /((\d+[+*-])*\d+)/g;
    ...
    let resVal = ''
    // 找到切割的最长算式
    while ((resVal = reg.exec(inputSrt)) != null) {
      const exp = resVal[0];
...
let inputSrt1 = ref('camila 66 70 46 158 80')
let inputSrt2 = ref('victoria 94 76 86 189 211')
let inputSrt3 = ref('anthony 29 17 83 21 48')
let inputSrt4 = ref('emily 53 97 1 19 218')
function fn(num,weights,inputSrt1,inputSrt2,inputSrt3,inputSrt4) {
  res.value = []
if (!(num&&weights&&inputSrt1&&inputSrt2&&inputSrt3&&inputSrt4)) return alert('请输入！')
  const weightArr = weights.split(' ')
  let inputArr = [inputSrt1.split(' '),inputSrt2.split(' '),inputSrt3.split(' '),inputSrt4.split(' ')]
  let map = []
...
<template>
  ...
  <input v-model='inputSrt1' /><br>
  <input v-model='inputSrt2' /><br>
  <input v-model='inputSrt3' /><br>
  <input v-model='inputSrt4' /><br>
  <button @click='fn(num,weights,inputSrt1,inputSrt2,inputSrt3,inputSrt4)' >点击运行</button>
  <h1>代码运行结果：{{ res }}</h1>
</template>
...
function fn(inputStr) {
  if (inputStr=='') return alert('请输入！') 
  let inputSrts = inputStr.split(' ')
  res.value = getResult(inputSrts[0],inputSrts[1])
...
let res = ref('')
let n = ref('4') // 成员总数
let inputSrt = ref('100 200 300 500') // 成员的财富值
let nodeInfo1 = ref('1 2') // 节点信息
let nodeInfo2 = ref('1 3') // 节点信息
let nodeInfo3 = ref('2 4') // 节点信息
function fn(n,inputSrt,nodeInfo1,nodeInfo2,nodeInfo3) {
if (inputSrt=='') return alert('请输入！')
  // 建立节点与财富值的映射，将0推到最前端，使节点（节点从1开始）与下标对应起来
  let nodes = inputSrt.split(' ').map(Number)
  nodes.unshift(0)
...
<template>
  ...
  <button @click='fn(n,inputSrt,nodeInfo1,nodeInfo2,nodeInfo3)' >点击运行</button>
  <h1>代码运行结果：{{ res }}</h1>
</template>
...
let res = ref('')
let num = ref('7')
let inputSrt = ref('8 4 6 3 1 6 7')
let val = ref('10')
function fn(inputSrt) {
if (inputSrt=='') return alert('请输入！')
  let arr = inputSrt.split(' ')
  let l = 0
  let r = 0
  let resArr = []
